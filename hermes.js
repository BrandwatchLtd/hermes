...
(function loadHermes() {
    function makeHermes($) {
        ...
        function makeNotification(style, message) {
            ...
            function animateIn(callback) {
                ...
                $li.addClass(style.in);

                $li.on(endEvents, function (event) {
                    event.stopPropagation();

                    $li.off();
                    $li.removeClass(style.in);
            ...
            function pause(callback) {
                ...
                $li.addClass(style.paused || '');

                pauseTimeout = setTimeout(function () {
                    pauseTimeout = null;

                    $li.removeClass(style.paused || '');
        ...
        function hermes(options) {
            ...
            if (options.listClasses) {
                $ul.addClass(options.listClasses);
            }
    ...
    if (typeof define === 'function' && define.amd) {
        return define(['jquery'], makeHermes);
    }
    ...
    if (!window.jQuery) {
        throw new Error('Hermes requires jQuery.');
    }

    window.hermes = makeHermes(window.jQuery);
}());
